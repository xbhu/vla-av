...
class NuScenesMapExplorer:
    ...
    def mask_for_polygons(polygons: MultiPolygon, mask: np.ndarray) -> np.ndarray:
        ...
        # Shapely 2.0: MultiPolygon is no longer directly iterable; normalize to a list of Polygons
        if hasattr(polygons, 'geoms'):
            polygon_list = list(polygons.geoms)
        elif isinstance(polygons, (list, tuple)):
            polygon_list = list(polygons)
            polygon_list = [p for sub in polygon_list for p in (list(sub.geoms) if hasattr(sub, 'geoms') else [sub])]
        else:
            polygon_list = [polygons]
        polygon_list = [p for p in polygon_list if not p.is_empty]
